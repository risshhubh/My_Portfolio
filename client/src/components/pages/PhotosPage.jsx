...
const PhotosPage = ({ darkMode = false }) => {
  ...
  const albums = [
    ...
    },
    {
      id: "pathwise",
      name: "Pathwise",
      description: "Pathwise - AI Job Portal screenshots",
      icon: "🧭",
      photos: [
        {
          src: getImagePath("/pathwise/Login.png"),
          alt: "Pathwise - Login screen",
        },
        {
          src: getImagePath("/pathwise/Home.png"),
          alt: "Pathwise - Home / Landing",
        },
        {
          src: getImagePath("/pathwise/Interview.png"),
          alt: "Pathwise - Interview page",
        },
        {
          src: getImagePath("/pathwise/Dashboard.png"),
          alt: "Pathwise - Dashboard",
        },

        {
          src: getImagePath("/pathwise/About.png"),
          alt: "Pathwise - About page",
        },

        {
          src: getImagePath("/pathwise/Profile.png"),
          alt: "Pathwise - Profile page",
        },
      ],
    },
  ];
  ...
  if (selectedAlbum) {
    return (
      <div
        className={`h-full w-full p-4 pb-24 overflow-y-auto bg-gradient-to-b ${
      ...
      >
        ...
        {selectedAlbum.photos.length === 0 ? (
          ...
          >
            ...
            <p className="text-sm">
              Add your Feasto project screenshots to showcase your work!
            </p>
            <div
            ...
            >
              <p className="text-xs">
                <strong>To add screenshots:</strong>
                <br />
                1. Place your images in the{" "}
                <code className="bg-gray-200 dark:bg-gray-700 px-1 rounded">
                  public/feasto/
                </code>{" "}
                folder
                <br />
                2. Update the photos array in this component
                <br />
            ...
            >
              <div
                ...
                className={`relative max-w-full max-h-full transition-all duration-700 ease-in-out transform ${
                  isFullScreen ? "scale-100 opacity-100" : "scale-95 opacity-90"
                }`}
              >
                {/* Close Button */}
                <button
                ...
                {currentPhotoIndex > 0 && (
                  <button
                    ...
                    className={`absolute left-4 top-1/2 transform -translate-y-1/2 z-10 p-3 rounded-full bg-black bg-opacity-50 text-white hover:bg-opacity-70 transition-all duration-300 ease-in-out ${
                      isFullScreen
                        ? "scale-100 opacity-100"
                        : "scale-90 opacity-80"
                    }`}
                ...
                {currentPhotoIndex < selectedAlbum.photos.length - 1 && (
                  <button
                    ...
                    className={`absolute right-4 top-1/2 transform -translate-y-1/2 z-10 p-3 rounded-full bg-black bg-opacity-50 text-white hover:bg-opacity-70 transition-all duration-300 ease-in-out ${
                      isFullScreen
                        ? "scale-100 opacity-100"
                        : "scale-90 opacity-80"
                    }`}
                ...
                {/* Photo Counter */}
                <div
                  className={`absolute bottom-4 left-4 right-4 bg-black bg-opacity-50 text-white p-3 rounded-lg transition-all duration-300 ease-in-out transform ${
                    isFullScreen
                      ? "scale-100 opacity-100 translate-y-0"
                      : "scale-95 opacity-80 translate-y-2"
                  }`}
                >
        ...
        {albums.map((album) => (
          ...
          >
            <div className="p-4 text-center">
              ...
              >
                {album.photos.length}{" "}
                {album.photos.length === 1 ? "screenshot" : "screenshots"}
              </div>
            </div>
          </div>
        ))}
